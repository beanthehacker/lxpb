...
| **Formation filter** | **LHPB is valid only if its formation bar is a HAMMER; LLPB is valid only if its formation bar is a SHOOTING STAR** — all other formations are dropped. Judged by patterns-pure's `find_hammer` / `find_shooting_star` (vendored in `../lxpb-v2/patterns_pure`, the only spike definition allowed in this repo; note this is the *opposite* polarity from `render_labels_report.is_spike_pp`, which maps LHPB→shooting-star/LLPB→hammer — this strategy instead matches lxpb.py's own native `is_spike` polarity, which uses the same patterns-pure functions). |
